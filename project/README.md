...
- **Краткое описание (2-4 предложения):**
  > Проект посвящен построения сервиса для обнаружения патологии на рентгеновских снимках грудной клетки. 
  > Используются открытый датасет Chest X-Ray Images (Pneumonia) (Kaggle), baseline-модели
  > В результате сервис работает на Fast API. Пользователь загружает изображение на сервер либо выбирает из доступных, и получает результат работы модели: найдена патология или нет.
...
- где лежат файлы:
  - `data/` – небольшая выборка для демонстрации;
...
> В репозитории лежит только небольшая подвыборка для демострации работы сервиса `data/sample/*`.
